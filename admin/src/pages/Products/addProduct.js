...
const ProductUpload = () => {
  ...
  const handleInputChange = (index, event) => {
    const values = [...formFields.specifications];
    values[index] = event.target.value; // Update the specific input value
    setFormFields({
      ...formFields,
      specifications: values // Update formFields with new specifications array
    });
  };
  ...
  const handleAddField = () => {
    setFormFields({
      ...formFields,
      specifications: [...formFields.specifications, ""] // Add a new empty specification
    });
  };
  ...
  const handleRemoveField = (index) => {
    const values = [...formFields.specifications];
    values.splice(index, 1); // Remove the specification at the given index
    setFormFields({
      ...formFields,
      specifications: values // Update formFields with the updated specifications array
    });
  };
  ...
  const addProduct = (e) => {
    ...
    img_arr = [];
    const keyValuePairs =formFields.specifications.reduce((acc, spec) => {
      const [key, value] = spec.split(":"); // Split by colon
      if (key && value) {
        acc[key.trim()] = value.trim(); // Store key-value pairs in an object
      }
      return acc;
    }, {});
              ...
              <div className="card p-4 mt-0">
                ...
                <div className="row">
                  <div className="col-md-12">
                    ...
                    <div>
      {formFields.specifications.map((specification, index) => (
        <div
          key={index}
          style={{
            display: "flex",
        ...
        >
          <input
            type="text"
            placeholder={`Key:Value (e.g., size:large)`}
            value={specification}
      ...
      {specifications.map((specification, index) => (
        <Box key={index} display="flex" alignItems="center" mb={2}>
          <input
            ...
            value={specification}
            onChange={(event) => handleInputChange(index, event)}
            style={{ flex: 1, marginRight: '8px', padding: '10px', borderRadius: '4px', border: '1px solid #ccc' }}
          />
          <Button onClick={() => handleRemoveField(index)} color="error" variant="outlined">
            Remove
          </Button>
        </Box>
      ))}
